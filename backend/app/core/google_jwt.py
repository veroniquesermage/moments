...
async def get_cached_jwks() -> dict:
    """
    Récupère les clés JWK Google avec cache en mémoire (TTL 5 minutes).
    Équivalent Java : Cache<String, Object> avec expireAfterWrite(5, MINUTES)
    """
    with tracer.start_as_current_span("google_jwks_cache_check") as span:
        ...
        if _jwks_cache is None or (now - _jwks_cache[1]) > CACHE_TTL:
            ...
            logger.debug("Cache JWK hit - Utilisation du cache")
